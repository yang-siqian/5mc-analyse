...
def _guppy(args, input_path=None, out_path=None):
    if input_path is None:
        input_path = args.input_path
    if out_path is None:
        out_path = args.out_path
    input_path = input_path.rstrip("/")
    num = out_path.rstrip("/").split('/')[-1]
    fastq_dir = out_path + "/{num}.guppy.fq".format(num=num)
...
def _deepsignal_call(args, input_path):
    if input_path is None:
        input_path = args.input_path
    input_path = input_path.rstrip("/")
    cmd = "deepsignal call_mods --input_path {input_path} --model_path {model_path} " \
          ...
          "--nproc {threads} --is_gpu {is_gpu}".format(input_path=input_path,
...
def run_guppy_deepsignal(args):
    ...
    if str2bool(args.is_guppy):
        ...
        fastq_dir, fastq_summary_txt = _guppy(args, input_path, args.out_path)
